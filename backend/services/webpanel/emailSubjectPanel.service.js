const EmailSubject = require("../../models/EmailSubject");
...
const methods = {
  ...
  async find(req) {
    ...
    try {
      const rows = await EmailSubject.find(_q.query)
        .sort({ sort: 1 })
        // .populate("userId")
        .exec(); // Populate the userId field with User document
      // .limit(limit)
      // .skip(offset);
      const count = await EmailSubject.countDocuments(_q.query);
      return {
        total: count,
  ...
  async findById(id) {
    try {
      const obj = await EmailSubject.findById(id);
  ...
  async insert(data) {
    try {
      const obj = new EmailSubject(data.body);
  ...
  async delete(id) {
    try {
      const obj = await EmailSubject.findById(id);
      if (!obj) return Promise.reject(ErrorNotFound("id: not found"));
      await EmailSubject.deleteOne({ _id: id });
  ...
  async update(id, data) {
    try {
      const obj = await EmailSubject.findById(id);
      if (!obj) return Promise.reject(ErrorNotFound("id: not found"));
      await EmailSubject.updateOne({ _id: id }, data);
