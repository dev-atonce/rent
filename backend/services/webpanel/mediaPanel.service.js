...
const methods = {
    ...
    /////// About us Image ///////
    async findAboutUs() {
        const dirPath = `./public/image/aboutUs`;
        const imagePath = [];
        try {
            // Check if the directory exists
            await fs.access(dirPath);
            ...
            const files = await fs.readdir(dirPath);
            files.map((file) => {
                imagePath.push(`${config.baseUrl}/public/image/aboutUs/${file}`);
            });
            return { data: imagePath, total: imagePath.length };
        } catch (error) {
            // If the error is about directory not existing, return an empty array
    ...
    async insertAboutUs(req, res) {
        const storage = multer.diskStorage({
            destination: (req, file, cb) => {
                const uploadDir = `./public/image/aboutUs/`;
                ensureDirectoryExistence(uploadDir);
                cb(null, uploadDir);
            },
            filename: (req, file, cb) => {
    ...
    async deleteMedia(req, res) {
        ...
        try {
            const deletePromises = files.map((fileUrl) => {
                try {
                    const filePath = path.join(process.cwd(), "public", new URL(fileUrl).pathname.replace(/^\/public\//, ""));
                    return fs.unlink(filePath);
                } catch (error) {
                    return Promise.reject(new Error(`Invalid URL: ${fileUrl}`));
                }
            });
